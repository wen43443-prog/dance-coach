...
用法:
  python deploy_github.py start    # 生成验证码，用户在 github.com/login/device 输入（首次）
  python deploy_github.py finish   # 自动部署：优先用已保存的授权，没有则等网页授权
  python deploy_github.py push     # 用已保存的授权重新上传（更新版本用）
"""
import base64
import hashlib
import json
...
DOC_FILES = [
    ...
    ("serve.py", os.path.join(HERE, "serve.py")),
    ("启动手机访问服务器.bat", os.path.join(HERE, "启动手机访问服务器.bat")),
    ("deploy_github.py", os.path.join(HERE, "deploy_github.py")),
    ("verify_deploy.py", os.path.join(HERE, "verify_deploy.py")),
    ("wait_deploy.py", os.path.join(HERE, "wait_deploy.py")),
    ("check_deploy.py", os.path.join(HERE, "check_deploy.py")),
    ("make_icons.py", os.path.join(HERE, "make_icons.py")),
]
...
def start():
    ...
    print("      python deploy_github.py finish")
    print("=" * 52)
...
def finish():
    ...
    while time.time() < deadline and not token:
        ...
        if st == 200 and resp.get("access_token"):
            ...
            sys.exit(1)
    if not token:
        print("验证码已过期，请重新运行: python deploy_github.py start")
        sys.exit(1)
...
def push():
    tok = load_saved_token()
    if not tok:
        print("没有已保存的授权，请先运行: python deploy_github.py finish")
        sys.exit(1)
    deploy_with_token(tok)
